...
class SAD:
    ...
    def cacualte(self,Sourse,target,mask):
        ...
        for index in indices:
            ...
            pairShow(Sourse,target,Posion(Tarbox),Posion(Srcbox),result=self.result)
            print('row:{}'.format(row))
        return self.result
        

    def __sad__(self,Src,Tar):
        return np.sum(np.abs(Src - Tar))
...
def cacualteSAD(Sourse,target,winSize,maxDisp):
    result = np.zeros_like(np.array(Sourse))
    indices = np.argwhere((target>0) == 1)    #row行索引  col列索引
    for index in indices:
        row,col = index
        try:
            del win
            ...
            pass
        win = SlidingWindowGenerator(Sourse)
        windowsList = list(win.generate_windows(col,row,winSize,maxDisp,maxDisp))

        winTarget,box = __targetCut__(target,winSize,col,row)
        if col <= winSize/2:   #这里第一个数据表示纵向坐标
            continue
        old = np.inf
        winResult = windowsList[0]
        for winIndex in windowsList:
            new = __sad__(winIndex.image,winTarget)
            if new < old:
                old = new
                winResult = winIndex
        ...
        Tarbox = winResult.box
        Srcbox = [(box[0],box[2]),(box[1],box[3])]
        result[row,col] = __distance__(Tarbox,Srcbox)
    return result

def __sad__(Src,Tar):
    return np.sum(np.abs(Src - Tar))

def __targetCut__(target,winSize,x,y):
    xSize = (winSize-1)/2
    box = [x - xSize,x + xSize+1,y - xSize,y + xSize+1]
    box = [int(i) for i in box ]
    return target[box[2]:box[3],box[0]:box[1]],box

def __distance__(tarbox,srcbox):
    return tarbox[0][0] - srcbox[0][0]
